...
import com.bunnbougu.app.service.OrderService;
...
public class OrderController extends BaseController implements HttpHandler {
        ...
        try {
            if (parts.length == 3) {
                if ("GET".equals(method)) {
                    ...
                    OrderRecord order = RequestUtil.readJsonBody(exchange, OrderRecord.class);
                    ApiResponse.sendJson(exchange, 201, service.create(order));
                } else {
            ...
            if ("PUT".equals(method)) {
                OrderRecord order = RequestUtil.readJsonBody(exchange, OrderRecord.class);
                OrderRecord updated = service.update(id, order);
                if (updated == null) {
                    ApiResponse.sendMessage(exchange, 404, "受注が見つかりません。");
                    return;
                }
